...
fn try_parse_message(buffer: &BytesMut) -> Option<(u16, usize)> {
    ...
    match msg_type {
        ...
        // Warp/room change: room_id (2) + x (2) + y (2)
        MSG_WARP => {
            if buffer.len() >= 8 {
                Some((msg_type, 8))
            } else {
...
async fn handle_message(
    ...
    match msg_type {
        ...
        MSG_WARP => {
            handle_warp(payload, server, session).await
        }
...
async fn handle_warp(
    ...
    let (player_id, old_room_id, character_id, body_id, acs1_id, acs2_id, username) = {
        let mut session_guard = session.write().await;
            ...
            if let Some(other_session_ref) = server.sessions.get(&other_session_id) {
                ...
                if other_session.is_authenticated {
                    if let Some(other_username) = &other_session.username {
                        ...
                        new_player_writer.write_u16(MSG_NEW_PLAYER)
