...
async fn handle_message(
    ...
    match msg_type {
        ...
        MessageType::Login => auth::handle_login(payload, server, session).await,

        MessageType::Register => auth::handle_register(payload, server, session).await,
        ...
        MessageType::MovePlayer => movement::handle_movement(payload, server, session).await,

        MessageType::Chat => chat::handle_chat(payload, server, session).await,

        MessageType::PlayerTyping => chat::handle_typing(server, session).await,

        MessageType::NewPlayer => chat::handle_new_player_response(payload, server, session).await,

        MessageType::Emote => chat::handle_emote(payload, server, session).await,

        MessageType::Action => chat::handle_action(payload, server, session).await,
        ...
        MessageType::DiscardItem => items::handle_discard_item(payload, server, session).await,

        MessageType::DiscardedItemTake => {
            items::handle_take_dropped_item(payload, server, session).await
        }

        MessageType::ShopBuy => shop::handle_shop_buy(payload, server, session).await,

        MessageType::SellReqPrices => shop::handle_sell_req_prices(payload, server, session).await,

        MessageType::Sell => shop::handle_sell(payload, server, session).await,

        MessageType::RequestStatus => bank::handle_request_status(payload, server, session).await,

        MessageType::BankProcess => bank::handle_bank_process(payload, server, session).await,

        MessageType::Mailbox => mail::handle_mailbox(payload, server, session).await,

        MessageType::MailSend => mail::handle_mail_send(payload, server, session).await,
        ...
        MessageType::BbsRequestGui => bbs::handle_bbs_request_gui(payload, server, session).await,
        ...
        MessageType::BbsRequestPost => bbs::handle_bbs_request_post(payload, server, session).await,

        MessageType::BbsPost => bbs::handle_bbs_post(payload, server, session).await,
            ...
            if let Ok(slot) = reader.read_u8() {
                ...
                if let Some(char_id) = character_id {
                    ...
                    if let Ok(Some(inventory)) = crate::db::get_inventory(&server.db, char_id).await
                    {
                        ...
                        if let Err(e) =
                            crate::db::update_equipped_tool(&server.db, char_id, slot as i16).await
                        {
        ...
        // Clan System
        MessageType::ClanCreate => clan::handle_clan_create(payload, server, session).await,

        MessageType::ClanDissolve => clan::handle_clan_dissolve(payload, server, session).await,

        MessageType::ClanInvite => {
            clan::handle_clan_invite_response(payload, server, session).await
        }

        MessageType::ClanLeave => clan::handle_clan_leave(payload, server, session).await,

        MessageType::ClanInfo => clan::handle_clan_info(payload, server, session).await,

        MessageType::ClanAdmin => clan::handle_clan_admin(payload, server, session).await,

        // Quest System
        MessageType::QuestBegin => quest::handle_quest_begin(payload, server, session).await,

        MessageType::QuestClear => quest::handle_quest_clear(payload, server, session).await,

        MessageType::QuestStepInc => quest::handle_quest_step_inc(payload, server, session).await,

        MessageType::QuestCancel => quest::handle_quest_cancel(payload, server, session).await,

        MessageType::QuestNpcReq => quest::handle_quest_npc_req(payload, server, session).await,

        MessageType::QuestReward => quest::handle_quest_reward(payload, server, session).await,

        MessageType::QuestVarCheck => quest::handle_quest_var_check(payload, server, session).await,

        MessageType::QuestVarInc => quest::handle_quest_var_inc(payload, server, session).await,

        MessageType::QuestVarSet => quest::handle_quest_var_set(payload, server, session).await,
        ...
        // Upgrader System
        MessageType::UpgraderGet => upgrader::handle_upgrader_get(payload, server, session).await,
...
async fn cleanup_session(
    ...
    if let Some(player_id) = player_id {
        ...
        server
            .game_state
            .remove_player_from_room(player_id, room_id)
            .await;
...
async fn check_and_broadcast_top_points(
    ...
    info!(
        "New top points holder: {} with {} total points",
        username, player_total
    );
