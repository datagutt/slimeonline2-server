...
pub async fn handle_upgrader_get(
    ...
    debug!("Upgrader GET: town={}, category={}, page={}", town_id, category, page);
    ...
    for slot_offset in 0..4u8 {
        ...
        if let Some(town) = town_config {
            if let Some(upgrade) = town.upgrades.get(&(category.to_string(), slot_id)) {
                ...
                let is_unlocked = match db::is_slot_unlocked(&server.db, town_id, category, slot_id).await {
...
pub async fn handle_upgrader_points(
    ...
    debug!("Upgrader POINTS: town={}, category={}, slot={}", town_id, category, slot_id);
    
    let mut writer = MessageWriter::new();
    writer.write_u16(MessageType::UpgraderPoints.id());
    
    // Get upgrade config from game_config
    let upgrade = server.game_config.upgrader.get_upgrade(town_id, category, slot_id);
...
pub async fn handle_upgrader_invest(
    ...
    debug!("Upgrader INVEST: town={}, category={}, slot={}, amount={}", 
    ...
    if current_points < invest_amount {
        warn!("Player {} tried to invest {} but only has {} points", 
    ...
    // Get upgrade config from game_config
    let upgrade = match server.game_config.upgrader.get_upgrade(town_id, category, slot_id) {
        Some(u) => u.clone(),
        None => {
            let mut writer = MessageWriter::new();
            writer.write_u16(MessageType::UpgraderInvest.id());
    ...
    if new_paid >= upgrade.need {
        info!("Upgrade completed! town={}, category={}, slot={}", town_id, category, slot_id);
        ...
        for chain_entry in &upgrade.unlock_chain {
            if let Err(e) = db::set_slot_unlocked(
                ...
                info!("Unlocked chained slot: category={}, slot={}", 
    ...
    match category {
        "Other" => {
            match upgrade.option {
                1 => {
                    ...
                    if let Err(e) = db::set_unlockable_available(&server.db, room_id, unlockable_id, true).await {
                ...
                2 => {
                    ...
                    if let Err(e) = db::set_music_unlocked(&server.db, room_id, song_slot, is_day).await {
                        warn!("Failed to set music unlocked: {}", e);
                    } else {
                        info!("Unlocked {} music slot {} in room {}", 
                              if is_day { "day" } else { "night" }, song_slot, room_id);
                    }
                }
                _ => {
        ...
        "Warp Center" => {
            ...
            if let Some(town) = town_config {
                ...
                if let Err(e) = db::set_warp_unlocked(&server.db, warp_room, warp_slot, warp_category).await {
                    warn!("Failed to set warp unlocked: {}", e);
                } else {
                    info!("Unlocked warp slot {} category {} in room {}", 
        ...
        "Outfit Shop" | "Acs Shop" | "Item Shop" => {
            match upgrade.option {
                1 => {
                    ...
                    if let Err(e) = db::set_shop_slot_unlocked(&server.db, shop_room, shop_slot).await {
...
pub async fn send_room_unlockables(
    server: &Arc<Server>,
    room_id: u16,
) -> Vec<Vec<u8>> {
    let mut responses = Vec::new();
