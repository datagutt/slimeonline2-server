    ...
    match element {
        1 => {
            ...
            if let Some(char_id) = character_id {
                ...
                writer
                    .write_u16(MessageType::RequestStatus.id())
...
pub async fn handle_bank_process(
    ...
    if !server
        .rate_limiter
        .check_player(session_id.as_u128() as u64, ActionType::Bank)
        .await
        .is_allowed()
    {
...
fn build_deposit_response(points: u32, bank: i64) -> Vec<u8> {
    let mut writer = MessageWriter::new();
    writer
        .write_u16(MessageType::BankProcess.id())
        .write_u8(1)
        .write_u32(points)
...
fn build_receiver_not_found_response() -> Vec<u8> {
    let mut writer = MessageWriter::new();
    writer.write_u16(MessageType::BankProcess.id()).write_u8(4);
    writer.into_bytes()
}
...
async fn handle_deposit(
    ...
    // Update database
    if let Err(e) =
        db::update_points_and_bank(&server.db, char_id, new_points as i64, new_bank).await
    {
    ...
    debug!(
        "Deposit OK: {} points -> bank. New points: {}, New bank: {}",
        amount, new_points, new_bank
    );
...
async fn handle_withdraw(
    ...
    debug!(
        "Withdraw OK: {} points <- bank. New points: {}, New bank: {}",
        amount, new_points, new_bank
    );
...
async fn handle_transfer(
    ...
    if (amount as i64) > sender_bank {
        warn!(
            "Transfer failed: player {} tried to transfer {} but only has {} in bank",
    ...
    if receiver.id == char_id {
        warn!(
            "Transfer failed: player {} tried to transfer to self",
            char_id
        );
        // Send current balance back to reset UI
        return Ok(vec![build_transfer_response(sender_bank)]);
    ...
    .await
    {
        Ok(()) => {
            debug!(
                "Transfer OK: {} points from {} to {}. Sender new bank: {}",
