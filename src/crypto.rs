...
//! We pre-compute the initial state to avoid running KSA (256 iterations) per message.
...
impl Rc4 {
    /// Create a new RC4 cipher with the given key (runs KSA - expensive)
    fn new(key: &[u8]) -> Self {
        ...
        // Key scheduling algorithm (KSA) - 256 iterations
        let mut j: u8 = 0;
        for i in 0..256 {
...
/// Pre-computed cipher state for decrypting client messages (avoids KSA per message)
static DECRYPT_CIPHER_TEMPLATE: LazyLock<Rc4> = LazyLock::new(|| Rc4::new(CLIENT_ENCRYPT_KEY));

/// Pre-computed cipher state for encrypting server messages (avoids KSA per message)
static ENCRYPT_CIPHER_TEMPLATE: LazyLock<Rc4> = LazyLock::new(|| Rc4::new(CLIENT_DECRYPT_KEY));
...
pub fn decrypt_client_message(data: &mut [u8]) {
    // Clone pre-computed state instead of running KSA
    let mut cipher = DECRYPT_CIPHER_TEMPLATE.clone();
...
pub fn encrypt_server_message(data: &mut [u8]) {
    // Clone pre-computed state instead of running KSA
    let mut cipher = ENCRYPT_CIPHER_TEMPLATE.clone();
    cipher.apply_keystream(data);
}
